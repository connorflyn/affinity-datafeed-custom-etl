...
# Create Post Request Query
datafeed_brands_query = "{ \"insights_name\":\""+insights_name+"\", " \
            "\"insights_type\":\"datafeed_brands\", " \
            "\"yyyymmdd_start\":"+start_date+", " \
            "\"yyyymmdd_end\":"+end_date+", " \
            "\"breakout_interval\":\""+breakout_interval+"\", " \
            "\"brand_ids\":"+brand_id_list+"} "

# Run Post request for datafeed_brands
datafeed_brands_response = run_datafeed_report_query(datafeed_brands_query)

# converts datafeed_brands Response to JSON Object
datafeed_brands_response = datafeed_brands_response.json()
print(f'[INFO] JSON datafeed_brands_response is:\n {datafeed_brands_response}\n\n')


# Denest datafeed_brands Response
datafeed_brands_filtered_response = datafeed_brands_response["data"]

# Create the Response Insight_ID variable (response_insight_id) - used to pull s3 file
response_insight_id = datafeed_brands_filtered_response["insights_id"]
# response_insight_id = 'xxxx'
...
# Check if insight report is ready for download
try:
    datafeed_brands_filtered_response = datafeed_brands_filtered_response["insights_status"]

    if not datafeed_brands_filtered_response == "complete":
        print("[WARNING] Sleeping the process for 5 minutes, the insight report is being prepared "
              "(currently 'queued' or 'pending)'...")
        print("[INFO] Starting sleep at: %s\n" % time.ctime())
        time.sleep(300)
        print(f"[INFO] Process finished sleeping, (insight status now '{datafeed_brands_filtered_response}')\n")
        if not datafeed_brands_filtered_response == "complete":
            print("[WARNING] Sleeping the process for 10 minutes, the insight report is being prepared "
                  "(currently 'queued' or 'pending)'...")
            print("[INFO] Starting sleep at: %s\n" % time.ctime())
            time.sleep(600)
            print(f"[INFO] Process finished sleeping, (insight status now '{datafeed_brands_filtered_response}')\n")
            if not datafeed_brands_filtered_response == "complete":
                print("[WARNING] Sleeping the process for 20 minutes, the insight report is being prepared "
                      "(currently 'queued' or 'pending)'...")
                print("[INFO] Starting sleep at: %s\n" % time.ctime())
                time.sleep(1200)
                print(f"[INFO] Process finished sleeping, (insight status now '{datafeed_brands_filtered_response}')\n")
            if not datafeed_brands_filtered_response == "complete":
                print("[ERROR] Insight results are not ready!\n")
        else:
            print("[INFO] Insight results ready now...\n")
            pass
    else:
        print("[INFO] Insight results ready now...\n")
        pass
